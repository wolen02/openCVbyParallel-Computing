    ...
    try:
        ...
        while not stop_event.is_set():
            ...
            if vw is None:
                ...
                vw = cv2.VideoWriter("/Users/wnwlt/Desktop/실험 결과/goodResult.mp4", fourcc, fps, (w, h))
...
def main():
        ...
        while not stop_event.is_set():
            ...
            while len(futures) < max_inflight and not stop_event.is_set():
                ...
                try:
                    ...
                    # 배치가 비어있으면 계속, 배치가 있으면 timeout 도달 체크
                    if batch and time.perf_counter() >= batch_deadline:
                        pass
                    else:
                        break
                ...
                batch.append((frame, meta))

                # 조건 만족하면 배치 flush
                if batch and (len(batch) >= batch_size or time.perf_counter() >= batch_deadline):
                    # 배치에 있는 프레임들을 한번에 submit
                    for frame, meta in batch:
                        futures.append(ex.submit(detect_person, frame, model, meta))
                    batch.clear()
                    batch_deadline = None

                    # inflight 제한 걸리면 밖으로 나가서 수거
    ...
    # 파일 경로 설정
    file_path = '/Users/wnwlt/Desktop/실험 결과/video_result4.xlsx'
